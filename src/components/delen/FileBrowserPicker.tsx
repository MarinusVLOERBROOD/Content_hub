...
import { Search, FileText, Image, Video, File, Check } from "lucide-react";
...
function getFirstLeafPath(nodes: FolderNode[]): string {
  if (nodes.length === 0) return "";
  const first = nodes[0];
  if (first.children.length > 0) return getFirstLeafPath(first.children);
  return first.path;
}
...
export function FileBrowserPicker({ open, onClose, onSelect, selectedIds }: FileBrowserPickerProps) {
  ...
  useEffect(() => {
    ...
    setFolders([]);
    setSelectedFolder("");
    fetch(`/api/clients/${activeClient}/folders`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    const url = selectedFolder
      ? `/api/clients/${activeClient}/files?folder=${encodeURIComponent(selectedFolder)}`
      : `/api/clients/${activeClient}/files`;
    fetch(url)
      .then((r) => r.json())
      .then((f) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Bestanden selecteren" width="max-w-3xl">
      <div className="flex h-96 gap-4">
        {/* Client list */}
        <div className="w-32 shrink-0 border-r border-slate-100 pr-3 overflow-y-auto">
            ...
            {filtered.map((f) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-800 truncate">{stripExtension(f.name)}</p>
